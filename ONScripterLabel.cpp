...
static void dtw_func_c(AnimationInfo::ONSBuf *buf, const ONSPixel::PixelFormat fmt, ONSrect &rect, int color[], const int colors, const int w)
{
...
//#else
    dtw_func->func = ::dtw_func_c; // TODO register additional accelerated versions
//#endif
...
void ONScripterLabel::displayTextWindow( SDL_Surface *surface, SDL_Rect &clip )
{
    ...
    // - is surface->pixels aligned to 16-byte boundaries? if not, can it be?
        ...
        dtw_func->func(buf, onsfmt, onsrect, color, ONS_FONT_COLORS, surface->w);
